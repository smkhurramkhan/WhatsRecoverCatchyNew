...
import com.catchyapps.whatsdelete.databinding.AudiosMediaFavouriteBinding
...
class FavAudiosMediaFragment : Fragment(), ActionMode.Callback {
    var adapterFavMedia: FavoriteVoiceAdapter? = null
    ...
    var binding: AudiosMediaFavouriteBinding? = null
    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        binding = AudiosMediaFavouriteBinding.inflate(inflater, container, false)
    ...
    private fun multiSelect(position: Int) {
        if (position > -1) {
            ...
            if (actionMode != null) {
                ...
                // Toast.makeText(getContext(), "Actin mode is null", Toast.LENGTH_LONG).show();
            }
        }
    }
        ...
        builder.setPositiveButton("DELETE") { dialog: DialogInterface, which: Int ->
            try {
                for (i in 0 until selectedIds.size()) {
                    ...
                    val fdelete = File(Objects.requireNonNull(recoverFileEntity.filePath))
                    if (fdelete.exists()) {
                        fdelete.delete()
                    }
        ...
        try {
            ...
            for (i in 0 until selectedIds.size()) {
                ...
                val u = FileProvider.getUriForFile(
                    ...
                    File(Objects.requireNonNull(uri))
                )
                selecteduri.add(u)
            }
            actionMode!!.title = ""
    ...
    fun playAudio(itemPos: Int) {
        ...
        binding!!.animationView.background =
            ContextCompat.getDrawable(requireContext(), R.drawable.round_circle)
        binding!!.totalTime.text = MessageFormat.format(" / {0}", getDuration(data.filePath))
        try {
            ...
            val tempFile = File(Objects.requireNonNull(data.filePath))
            ...
            mediaPlayer?.setOnCompletionListener { mediaPlayers: MediaPlayer? ->
                mediaPlayer!!.seekTo(0)
                binding!!.realseekBar.progress = 0
                isPlaying = false
                binding!!.runTime.setText(R.string._00_0000)
                binding!!.playButton.background =
                    ContextCompat.getDrawable(requireContext(), R.drawable.icon_primary_play)
            }
        } catch (e: Exception) {
            e.printStackTrace()
